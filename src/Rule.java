...
public class Rule {
    ...
    public static void beforeMove(Square selecting) {
        ...
        Rule.enableAll(getCheckFilteredMove(selecting));   
    }
            ...
            for (Square[] col : board)
                for (Square row : col)
                    if(row.getChess() != null 
                        ...
                        ((Pawn)selectedSquare.getChess()).setEnPassant(false);
        }
    ...
    static public ArrayList<Square> getBasicMove(Square square){
        int[][] movement;
        switch (square.getChess().getClass().getName()) {
        case "King":
            movement = new int[8][2];
            for (int i = 0; i < movement.length; i++) {
                    movement[i] = NORMAL_MOVE[i];
            }
            return getRuleFilteredMove(square, movement, 1);
        case "Pawn":
            movement = new int[3][2];
            if (square.getChess().isRed()) {
                movement[0] = NORMAL_MOVE[1];
                movement[1] = NORMAL_MOVE[3];
                movement[2] = NORMAL_MOVE[2];
            } else {
                movement[0] = NORMAL_MOVE[7];
                movement[1] = NORMAL_MOVE[5];
                movement[2] = NORMAL_MOVE[6];
            }
            return getRuleFilteredMove(square, movement, ((Pawn)square.getChess()).isFirstRound() ? 2 : 1);
        case "Rook":
            movement = new int[4][2];
            for (int i = 0, j = 0; i < movement.length; i++, j += 2) {
                movement[i] = NORMAL_MOVE[j];
            }
            return getRuleFilteredMove(square, movement, 7);
        case "Bishop":
            movement = new int[4][2];
            for (int i = 0, j = 1; i < movement.length; i++, j += 2) {
                movement[i] = NORMAL_MOVE[j];
            }
            return getRuleFilteredMove(square, movement, 7);
        case "Knight":
            return getRuleFilteredMove(square, KNIGHT_MOVE, 1);
        case "Queen":
            return getRuleFilteredMove(square, NORMAL_MOVE, 7);
        }
        return null;
    }
    ...
    static public ArrayList<Square> getCheckFilteredMove(Square selecting) {
        ArrayList<Square> movelist = Rule.getBasicMove(selecting);
